...
def calc_quat_angle_error(label, pred):
    q1 = pred / np.linalg.norm(pred, axis=0, keepdims=True)
    q2 = label / np.linalg.norm(label, axis=0, keepdims=True)
    d = np.abs(np.sum(np.multiply(q1,q2), axis=0, keepdims=True)) # Here we have abs()
...
def calc_quat_angle_error_single(label, pred):
    q1 = pred / np.linalg.norm(pred)
    q2 = label / np.linalg.norm(label)
